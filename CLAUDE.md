...
| Documents (`js/documents.js`) | The Letter of Intent (kind `intent`; earlier rows `cover`), numbered `AQT/INT/YYMMXXX` (sequence per month, file name swaps `/` for `-`), drawn in the browser with pdf-lib on the ADspace letterhead (the reference is the End Thank You Letter): wordmark in Optima, Co. Reg. and address left, the monogram and phone, email, website right; PRIVATE & CONFIDENTIAL; Our Ref / Date (12th September 2026) / To (registered name in capitals) / Attn (billing contact, role); the subject LETTER OF INTENT; Dear {contact}; the intent paragraph, the enquiry, the lines table, the totals, the totals sentence, a mute line of deal facts; Yours sincerely, ADSPACE PLT, the signed-in person's team name; Company Profile QR (https://go.adspace.me/co) bottom right; the monogram bottom centre and Page x of y bottom right on every page. Stored as issued in `client_documents`, Void then Delete. |
...
- Brand assets: header mark https://mycdn.adspace.me/adspace-brandname.png (`brandLogo`), favicon and og:image https://mycdn.adspace.me/adspace-favicon.png; in the repo for print: `css/adspace-mark.png` (the monogram, with alpha, taken from the reference letter), `css/adspace-profile-qr.png` (the Company Profile QR, https://go.adspace.me/co), `css/SlateBook.TTF` (text), `css/SlateRg.TTF` (heavier face), `css/OPTIMA.TTF` (the wordmark). Older `ADspace.png` and the website favicon were rejected.
