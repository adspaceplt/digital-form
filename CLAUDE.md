...
| Team-only policies (`is_team()`) | Clients have logins too, so `authenticated` no longer means the team: `team_members`, `team_roles`, `services`, `activity_viewers` reads, `activity_log` writes, the `content` storage bucket, `delete_client` and both edge functions (`sign-upload`, `invite-member`) check an active team row. |
