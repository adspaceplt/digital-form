...
| Documents (`js/documents.js`) | The Letter of Intent (kind `intent`; earlier rows `cover`), numbered `AQT/INT/YYMMXXX` (sequence per month, file name swaps `/` for `-`), drawn in the browser with pdf-lib: mark, title, reference, date, prepared by (the signed-in person's team name), account owner, Client and Billing contact blocks, the deal facts, a statement in sentences of what the lead needs (intent, enquiry, each line with quantity, months, start date, rate, amount and state, the totals and SST), the lines table, totals, Prepared by / Checked by. Stored as issued in `client_documents`, Void then Delete. |
...
- Brand assets: mark https://mycdn.adspace.me/adspace-brandname.png (`brandLogo`; to be committed as `css/adspace-brandname.png` for the PDF), favicon and og:image https://mycdn.adspace.me/adspace-favicon.png, brand fonts in the repo at `css/SlateBook.TTF` (text), `css/SlateRg.TTF` (heavier face), `css/OPTIMA.TTF`. Older `ADspace.png` and the website favicon were rejected.
- Issuer (`ADSPACE_ORG` in `js/config.js`, entered by the user): ADSPACE PLT, registration 202304002162, SST J31-2401-32100002, 61-02, Jalan Mutiara Emas 2A, Taman Mount Austin, 81100 Johor Bahru, Johor, Malaysia, advertise@adspacestudios.com, 60187625233. Bank line still blank.
