...
- **2026-09-15 — the reported exposure was not real, and the report was the defect.** A HIGH severity finding was raised from reading `supabase/schema.sql`, which creates `for all to authenticated using (true)` on fifteen tables. The live database had been hardened against that already, under the names `_read`, `_write`, `_edit` and `_del`, gated on the group flag that owns each table, with delete additionally asking for `remove` — stricter than the replacement being proposed, which would have loosened delete on fourteen of them. **The repository is not the database. A finding about production is verified against production before it is reported.** The preflight is what caught it, and its first version compared policy *names* against a pattern of its own, so it called forty-five correct policies UNREVIEWED; it compares table, name, role, command, `USING`, `WITH CHECK`, the feature permission and the `remove` requirement now, because a name grants nothing.
- **2026-09-15 — open, low.** `anon` holds DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE and UPDATE directly on all fifteen tables. RLS denies it, because no policy names that role, so this is defence in depth rather than a live hole. The migration revokes it; not yet applied.
- **2026-09-15 — open, high if it ever happens.** `supabase/schema.sql` still creates the permissive policies earlier in the file. Until the branch merges, a full re-run of that file would drop the live hardening and reinstate the exposure on all fifteen tables. The block at the foot of the file closes this; it is the reason the branch exists.
