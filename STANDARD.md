...
- **2026-09-22** — `css/SlateMed.TTF` is named by `ADSPACE_ORG.fontMed` and is **not in the repository**; the engine falls back to Slate Regular for the headings until the user adds the file. The Chinese block of a quotation cover embeds Noto Sans TC from jsDelivr (`ADSPACE_ORG.fontCjk`), because Slate carries no Chinese; a letter with a Chinese block is refused by name when that face cannot be fetched.
