...
- **2026-09-24 — My Work as a daily task list, returned for review and not merged.** Built from the user's brief "Reframe My Work as lightweight daily task tracking" on top of the phase 4 branch work: views (My day default), group by, five plain statuses read off `stage_group`, a completion tick for everyday tasks and a progress ring for workflow tasks, inline owner and due date, a drawer with one next action and Open full record, a three-field Add task, templates, a workload view, and the full record reduced to one derived state and one next step. **One migration to run by hand**: `supabase/migrations/2026-09-24-my-work-daily-tasks.sql` (the Everyday task workflow, template columns, `ops_create_task` and `ops_transition_task` redefined, and five new functions; mirrored in `supabase/schema.sql` and compared by `tests/ops.js`). **Skipping a step is now ops Manage** (`override-denied`), a deliberate tightening: a skip passes over a gate. A backward move's reason is asked on the page only. Existing tasks, their stages, events and permissions are unchanged. Verified by `ops: ok`, `sql: ok`, `work: ok`, `matrix: ok` and `uxaudit: ok`.
