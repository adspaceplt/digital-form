...
- **2026-09-22, NOT BUILT** — Splitting the Activity record's access per section (`activity.campaigns`, `activity.clients`, …) so one section's log can be opened to the team while the rest stays gated. The parts model already supports it; what it needs is a tag-to-section map in the database, which lives in `js/admin.js`'s `ACTION_LABEL` today with about thirty tags whose section does not follow their prefix. Duplicating that map in SQL is the drift this repository has ruled out twice, so it needs a generated function compared byte for byte the way the operations schema is. Its own batch.
