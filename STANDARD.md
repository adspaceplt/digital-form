...
- **2026-09-24 — One split per form; Cmd + Enter.** Rows of fields use `.fgrid` equal columns, never per-field pixel widths (the Billing pane split at three different points); measured by `tests/geom.js`. Cmd/Ctrl + Enter presses the nearest main action and never a destructive one (`tests/keys.js`). **Decided the same day, to build next**: the site-wide form system from the audit — secondary buttons tonal grey, defaults folded under More details on every create sheet, Billing and Brand read-first with Edit, forms in titled sections, segments for two to four choices (the user: "Proceed all three").
